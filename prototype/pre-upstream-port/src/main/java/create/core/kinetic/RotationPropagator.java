...
public final class RotationPropagator {
    ...
    private static void clearNetwork(IKineticTile te, Set<MyBlockPos> cleared) {
        ...
        while (!queue.isEmpty()) {
            ...
            // Notify client: speed=0 must be synced or TESR keeps spinning
            if (current instanceof net.minecraft.tileentity.TileEntity) {
                net.minecraft.tileentity.TileEntity curTe =
                        (net.minecraft.tileentity.TileEntity) current;
                if (curTe.getWorldObj() != null) {
                    curTe.getWorldObj().markBlockForUpdate(curTe.xCoord, curTe.yCoord, curTe.zCoord);
                }
            }
